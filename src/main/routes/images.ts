...
router.post("/generate", async (req: ProjectRequest, res) => {
  ...
  try {
    ...
    const result = await generateImageFn(
      genOptions as Parameters<typeof generateImageFn>[0],
    );
...
router.get("/:imageId", async (req: ImageRequest, res) => {
  ...
  const [image] = await db
    ...
    .where(
      and(
        eq(images.id, req.params.imageId),
...
router.patch("/:imageId", async (req: ImageRequest, res) => {
  ...
  await db
    .update(images)
    .set({ name: safeName, path: newImagePath, updatedAt: new Date() })
  ...
  res.json({ id: image.id, name: safeName });
});
