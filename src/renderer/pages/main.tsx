...
import { api_get, api_post, api_delete, isApiError } from "@/lib/api";
import {
  AlertTriangleIcon,
  FilesIcon,
  FolderIcon,
...
export default function Main() {
  ...
  async function handleSelectDirectory() {
    try {
      const data = await api_post<{ path: string | null }>(
      ...
      .catch((err) => {
        ...
        if (isApiError(err)) {
          const responseErrors = (
            err.response as { errors?: Record<string, string[] | string> }
          )?.errors;
    ...
    <div className="flex h-screen flex-col">
              ...
              {projects.map((project) => (
                <li key={project.id} className="relative">
                  <Link
                    ...
                    className="group border-border block h-full rounded-xl border bg-white p-5 transition hover:border-neutral-300 hover:shadow-sm dark:bg-neutral-950 dark:hover:border-neutral-700"
                  >
                    ...
                    <div className="mt-6 flex items-end justify-between gap-3">
                      <div className="flex min-w-0 flex-1 items-center gap-2">
                        <span className="border-border inline-flex shrink-0 items-center gap-1.5 rounded-md border bg-neutral-50 px-2 py-1 text-xs font-medium text-neutral-600 dark:bg-white/5 dark:text-neutral-300">
                        ...
                        {project.filesystem_root && (
                          <span
                            className="border-border inline-flex min-w-0 items-center gap-1.5 rounded-md border bg-neutral-50 px-2 py-1 text-xs font-medium text-neutral-600 dark:bg-white/5 dark:text-neutral-300"
          ...
          <form onSubmit={handleSubmit} className="grid gap-4">
            ...
            <div className="grid gap-2">
              ...
              <Textarea
                ...
                onChange={(e) => form.setData("initial_prompt", e.target.value)}
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {trashEnabled ? "Move to Trash?" : "Delete project?"}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {trashEnabled
