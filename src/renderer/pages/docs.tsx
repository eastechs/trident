...
function DocsView({
  ...
  // location.state.focusDocumentId; on first arrival we look the doc up and
  // open it as a tab. consumedRef stops the same id from re-firing if the
  // documents list changes (e.g. after a save) while the state is still set.
  const location = useLocation();
  const focusDocumentId =
    (location.state as { focusDocumentId?: string } | null)?.focusDocumentId ??
    null;
  const consumedFocusRef = useRef<string | null>(null);
  useEffect(() => {
    if (!focusDocumentId || consumedFocusRef.current === focusDocumentId)
      return;
    const doc = localDocuments.find((d) => d.id === focusDocumentId);
    if (doc) {
      openDocument({ id: doc.id, name: doc.name });
      consumedFocusRef.current = focusDocumentId;
    }
  }, [focusDocumentId, localDocuments, openDocument]);
  ...
  const addDocument = useCallback(() => {
    ...
    api_post<{ id: string; filename: string }>(
      ...
      .then((data) => {
        setTabs((prev) => [...prev, { id: data.id, title: data.filename }]);
  ...
  const submitRename = useCallback(
    (docId: string) => {
      ...
      api_patch<{ id: string; name: string }>(
        `/api/projects/${project.id}/documents/${docId}`,
        {
        ...
        .then((data) => {
          setTabs((prev) =>
            prev.map((t) => (t.id === docId ? { ...t, title: data.name } : t)),
          );
          setLocalDocuments((prev) =>
            prev.map((d) => (d.id === docId ? { ...d, name: data.name } : d)),
          );
        })
        .catch((error) => {
        ...
        <div className="flex items-center gap-2">
          ...
          {activeDocument?.last_edited_by && (
            <span className="rounded bg-neutral-50 px-1.5 py-0.5 text-[10px] text-neutral-400 dark:bg-neutral-900 dark:text-neutral-500">
              {activeDocument.last_edited_by === "user"
                ? "You"
                : activeDocument.last_edited_by}
            </span>
          )}
