...
function readPanelState(projectId: string, side: 'left' | 'right'): PanelState | null {
    try {
...
function writePanelState(projectId: string, side: 'left' | 'right', state: PanelState): void {
    try {
        const saved = localStorage.getItem(getStorageKey(projectId));
...
export function ChatPanel({
    ...
    const [activeConversationId, setActiveConversationId] = useState<string | null>(() => {
        if (savedState?.activeId && conversations.some((c) => c.id === savedState.activeId)) {
    ...
    useEffect(() => {
        writePanelState(projectId, side, { activeId: activeConversationId, tab: activeTab });
    ...
    const handleSelectConversation = useCallback((id: string) => {
        ...
        api_patch(`/api/projects/${projectId}/conversations/${id}`, { side }).catch(() => {});
    }, [projectId, side]);
    ...
    const confirmDelete = useCallback(() => {
        ...
        api_delete(`/api/projects/${projectId}/conversations/${id}`)
            ...
            .catch(console.error);
    }, [deletingId, projectId, onConversationDeleted, activeConversationId, conversations, otherSideActiveId, handleNewChat]);
