...
export function ChatPanelHeader({
  ...
  return (
    <div className="flex items-center gap-1.5 border-b px-2 py-1.5">
      <Tabs
        value={activeTab}
        onValueChange={(v) => onTabChange(v as "chat" | "history")}
      ...
      >
        <TabsList className="h-7 w-full justify-start bg-neutral-50 dark:bg-neutral-900">
          ...
          {hasConversation && (
            <TabsTrigger
              value="chat"
              className="h-full min-w-0 flex-1 justify-start overflow-hidden px-2"
            >
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <button
              type="button"
              onClick={onNewChat}
              className="text-muted-foreground hover:bg-muted hover:text-foreground flex shrink-0 items-center justify-center rounded-md p-1 transition-colors"
            >
