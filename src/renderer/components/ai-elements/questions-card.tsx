...
export function QuestionsCard({
  ...
  const canProceed =
    ...
    (currentAnswer.type === "option" ||
      (currentAnswer.type === "custom" &&
        currentAnswer.value.trim().length > 0));
  ...
  return (
    <div className="not-prose border-border bg-card w-full rounded-xl border p-5">
      ...
      <h3 className="text-foreground mb-5 leading-snug font-semibold">
          ...
          <div className="min-w-0 flex-1">
            ...
            {isCustomSelected && (
              <textarea
                ...
                rows={2}
                className="border-border bg-background text-foreground placeholder:text-muted-foreground focus:border-ring mt-2 w-full resize-none rounded-md border p-2.5 text-xs focus:outline-none"
              />
            )}
          </div>
        </button>
      ...
      <div className="mt-5 flex items-center justify-between">
        {currentIndex > 0 ? (
          <button
            type="button"
            onClick={handleBack}
            className="border-border text-foreground hover:bg-accent rounded-lg border px-5 py-2 text-xs font-medium transition-colors"
          >
            Back
          </button>
        ) : (
